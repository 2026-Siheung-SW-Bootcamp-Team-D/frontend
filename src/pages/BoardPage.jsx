  ...
  return <main className="flex min-h-screen flex-col bg-bg">
    ...
    <div className="grid flex-1 lg:grid-cols-[minmax(0,1fr)_420px]">
      <section className="relative min-h-[360px] overflow-hidden bg-[#d7e5df] p-5">
        ...
        <KakaoMap className="absolute inset-0" center={focusedPlace ? { lat: focusedPlace.lat, lon: focusedPlace.lon } : undefined} markers={[...places, ...searchLayer, ...(pickedPoint ? [{ id: "picked-point", name: "선택한 위치", ...pickedPoint }] : [])]} polygons={areaVisible && selectedArea ? [{ id: selectedArea.id, geometry: selectedArea.commonArea }] : []} selectedMarkerId={focusedId ?? board?.selectedPlaceId} onMarkerSelect={(place) => { if (place.kind === "search") setSearchLayer([{ ...place, id: "search-selected" }]); else setFocusedId(place.id); }} onMapClick={(point) => { if (pinMode) setPickedPoint(point); }} />
        <button type="button" onClick={() => { setAddingPlace(true); setPickedPoint(null); setSearchLayer([]); }} className="absolute bottom-5 right-5 z-10 rounded-xl bg-coral px-4 py-3 font-bold text-white">＋ 장소 추가</button>
        ...
        {addingPlace && <AddPlacePanel boardId={boardId} reload={reload} pickedPoint={pickedPoint} onLayerChange={setSearchLayer} onPickMode={setPinMode} onClose={() => { setAddingPlace(false); setSearchLayer([]); setPinMode(false); setPickedPoint(null); }} />}
      </section>
