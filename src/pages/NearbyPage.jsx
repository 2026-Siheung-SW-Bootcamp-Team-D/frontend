...
const CATEGORIES = [["RESTAURANT", "음식점"], ["CAFE", "카페"], ["PLAY", "놀거리"]];
...
export function NearbyPage({ boardId, initialLat, initialLon }) {
  ...
  async function search(nextCategory = category) {
    ...
    if ((q && nextCategory) || (!q && !nextCategory)) {
    ...
    setStatus("loading");
    try {
      const found = await searchNearbyPlaces(boardId, { ...point, q: q || undefined, category: nextCategory || undefined }, { signal: controller.signal });
  ...
  async function add(item) {
    ...
    try {
      ...
      if (!requestError?.isCanceled) setError(messageFor(requestError));
    } finally {
      setAddingId("");
    }
  }
