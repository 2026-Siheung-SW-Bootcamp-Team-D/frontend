...
import { moveCoursePlace, removeCoursePlace } from "../features/course/courseModel";
...
export function CoursePage({ boardId }) {
  ...
  return <main className="flex min-h-screen flex-col bg-bg">
    ...
    <section className="relative h-[36dvh] min-h-64 overflow-hidden bg-sky-soft lg:h-[calc(100dvh-61px)] lg:flex-1">
      <KakaoMap className="absolute inset-0" center={focusedPlace ? { lat: focusedPlace.lat, lon: focusedPlace.lon } : undefined} markers={places.map((place) => ({ ...place, order: orderByPlaceId.get(place.id) }))} selectedMarkerId={focusedPlace?.id} placeOverlay={overlayPlace} onOverlayClose={() => setFocusedPlaceId(null)} onOverlayDetail={(place) => navigate(`/boards/${boardId}/places/${place.id}`)} onMarkerSelect={(place) => setFocusedPlaceId(place.id)} />
      <div className="absolute left-4 top-4 z-10 rounded-full bg-white/95 px-3 py-2 text-sm font-black shadow-sm">우리 모임 코스예요!</div>
    </section>
    <section className="mx-auto w-full max-w-3xl px-4 py-5 lg:absolute lg:bottom-0 lg:right-4 lg:max-h-[calc(100dvh-85px)] lg:w-[420px] lg:overflow-y-auto lg:rounded-3xl lg:bg-white lg:shadow-2xl">
      ...
      <section className="mt-5 rounded-2xl bg-sky-soft p-4"><div className="flex items-center justify-between gap-3"><div><h2 className="font-black">첫 만남 장소 이동시간</h2><p className="mt-1 text-xs text-ink-2">출발지는 공개되지 않아요.</p></div><button type="button" disabled={transitLoading} className="rounded-xl bg-navy px-3 py-2 text-sm font-black text-white disabled:opacity-50" onClick={calculateTransit}>{transitLoading ? "계산 중…" : "계산하기"}</button></div>{transitTimes.length > 0 && <div className="mt-3 space-y-2">{transitTimes.map((item) => <div key={item.participantId} className="flex items-center gap-2 rounded-xl bg-white p-3"><span className="grid h-8 w-8 place-items-center rounded-[10px] text-xs font-black text-white" style={{ backgroundColor: item.avatarColor }}>{item.nickname[0]}</span><b className="min-w-0 flex-1 truncate text-sm">{item.nickname}</b><span className="text-right text-sm">{transitLabel(item)}</span></div>)}</div>}</section>
    </section>
  </main>;
}
