...
export function AddPlacePanel({ boardId, reload, onClose, onLayerChange, pickedPoint, onPickMode }) {
  ...
  async function search(event) {
    ...
    try {
      const next = nearbyOnly && pickedPoint
        ? await searchNearbyPlaces(boardId, { ...pickedPoint, q: value, radius: 1500 }, { signal: controller.signal })
  ...
  return <section className="absolute inset-x-0 bottom-0 z-30 max-h-[78%] overflow-auto rounded-t-3xl bg-white p-5 shadow-2xl lg:inset-y-0 lg:left-auto lg:w-[420px] lg:max-h-none lg:rounded-none">
    ...
    {mode === "search" ? <><form onSubmit={search} className="flex gap-2"><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-line p-3" placeholder="장소명 또는 지역 검색" /><Button type="submit" disabled={busy} className="!w-auto !px-4 !py-3">{busy ? "검색 중" : "검색"}</Button></form><label className="mt-3 flex items-center justify-between rounded-xl bg-bg p-3 text-sm"><span>{pickedPoint ? "선택 위치 주변 1.5km" : "지도에서 위치를 선택해 주변 검색"}</span><input type="checkbox" disabled={!pickedPoint} checked={nearbyOnly} onChange={(event) => setNearbyOnly(event.target.checked)} /></label><label className="mt-2 flex items-center justify-between rounded-xl bg-bg p-3 text-sm"><span>검색 결과 레이어</span><input type="checkbox" checked={layerVisible} onChange={(event) => { setLayerVisible(event.target.checked); onLayerChange(event.target.checked ? results.map((item, index) => ({ ...item, id: `search-${item.providerPlaceId || index}`, kind: "search" })) : []); }} /></label><div className="mt-4 space-y-2">{results.map((item, index) => <button key={`${item.providerPlaceId}-${index}`} type="button" onClick={() => pick(item)} className={`w-full rounded-2xl border p-3 text-left ${selected === item ? "border-coral bg-coral-soft" : "border-line"}`}><b className="block">{item.name}</b><small className="text-ink-2">{item.address || item.category}</small></button>)}</div>{selected && <Button disabled={busy} className="mt-4" onClick={() => save(selected)}>{busy ? "추가 중…" : "선택한 장소 추가"}</Button>}</> : <><p className="rounded-xl bg-bg p-3 text-sm text-ink-2">지도의 원하는 지점을 눌러 주세요. 좌표 입력은 필요 없어요.</p>{pickedPoint && <><input value={manualName} onChange={(event) => setManualName(event.target.value)} className="mt-3 w-full rounded-xl border border-line p-3" placeholder="장소 이름" /><p className="mt-2 text-sm text-ink-2">{manualAddress}</p><Button disabled={busy || !manualName.trim()} className="mt-4" onClick={() => save({ name: manualName.trim(), roadAddress: manualAddress, lat: pickedPoint.lat, lon: pickedPoint.lon, sourceProvider: "MANUAL", inputMethod: "MANUAL_PIN" })}>{busy ? "추가 중…" : "이 위치 추가"}</Button></>}</>}
  </section>;
}
