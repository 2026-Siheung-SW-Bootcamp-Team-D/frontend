...
export function AddPlacePanel({ boardId, reload, onClose, onLayerChange, pickedPoint, markerSelection, radius, onRadiusChange, onPickMode }) {
  ...
  async function search(event, categoryOverride = category) {
    ...
    if (!categoryOverride && (value.length < 2 || value.length > 60)) return setError("검색어는 2~60자로 입력해 주세요.");
    const controller = new AbortController();
    controllerRef.current?.abort(); controllerRef.current = controller;
    setBusy(true); setError("");
    try {
      const nearbyRequest = categoryOverride === "ACTIVITY"
        ? { ...pickedPoint, theme: "ACTIVITY", radius }
        : categoryOverride ? { ...pickedPoint, category: categoryOverride, radius } : { ...pickedPoint, q: value, radius };
      const next = (nearbyOnly || categoryOverride) && pickedPoint
        ? await searchNearbyPlaces(boardId, nearbyRequest, { signal: controller.signal })
        : await searchPlaces(boardId, value, {}, { signal: controller.signal });
      if (!controller.signal.aborted) {
        setResults(next); setSelected(next[0] ?? null); setLayerVisible(true);
        onLayerChange(next.map((item, index) => ({ ...item, id: `search-${item.providerPlaceId || index}`, kind: "search" })));
      }
  ...
  async function save(place) {
    ...
    setBusy(true); setError("");
    try {
      await createPlace(boardId, { name: place.name, category: place.category || "직접 지정", roadAddress: place.roadAddress || null, jibunAddress: place.jibunAddress || null, location: { lat: place.lat, lon: place.lon }, source: { sourceProvider: place.sourceProvider || "KAKAO", providerPlaceId: place.providerPlaceId || null, sourceUrl: place.sourceUrl || null, inputMethod: place.inputMethod || "SEARCH_PICK" } }, { signal: controller.signal });
  ...
  return <section className="absolute inset-x-0 bottom-0 z-30 max-h-[78%] overflow-auto rounded-t-3xl bg-white p-5 shadow-2xl lg:inset-y-0 lg:left-auto lg:w-[420px] lg:max-h-none lg:rounded-none">
    ...
    {mode === "search" ? <>
      ...
      {activeSelection && <Button disabled={busy} className="mt-4" onClick={() => save(activeSelection)}>{busy ? "추가 중…" : `“${activeSelection.name}” 추가하기`}</Button>}
