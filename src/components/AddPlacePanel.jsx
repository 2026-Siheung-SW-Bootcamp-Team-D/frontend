...
export function AddPlacePanel({ boardId, reload, onClose, onLayerChange, pickedPoint, onPickMode }) {
  ...
  async function search(event) {
    ...
    try { const next = await searchPlaces(boardId, value, {}, { signal: controller.signal }); if (!controller.signal.aborted) { setResults(next); setSelected(next[0] ?? null); onLayerChange(next.map((item, index) => ({ ...item, id: `search-${item.providerPlaceId || index}`, kind: "search" }))); } }
